...
Helsel and Hirsch (2002) describe multiple regression with periodic functions to model seasonal variability in section 12.4.3. They describe the approach by adding two variables, one for the sine and one for the cosine of annual time. The \texttt{fourier} function in \texttt{smwrBase} simplifies the addition of those variables by adding both sine and cosine transformations to the model. It also forces both or neither to be considered in best-subset selection procedures. The \texttt{k.max} argument can be used to add higher-order seasonal terms; the \texttt{k.max} argument is the number of cycles per year.
...
\bibitem{HH2}
Helsel, D.R., and Hirsch, R.M., 2002, Statistical methods in water resources: U.S. Geological Survey Techniques of Water-Resources Investigations, book 4, chap. A3, 522 p. Available online at: http://pubs.usgs.gov/twri/twri4a3/pdf/twri4a3-new.pdf.
