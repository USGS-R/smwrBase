...
These examples demonstrate how to use functions with the \texttt{USGSwsBase} package that transform explanatory variables to help model response-explanatory variable relations commonly found in hydrologic data. These example use a single explanatory variable with synthetic data to illustrate how to model the relations. As with most hydrologic relations, the data are assumed to be log-log related.

These examples assume that the user is familiar with general knowledge about linear regression and how to build models in R. The examples simply illustrate some functions in the \texttt{USGSwsBase} package without any recommendation of application.
...
<<fig=TRUE, echo=FALSE>>=
with(DF, plot(Flow, Y1, type='p', pch=16, tck=.02, log="xy"))
...
Second-order fits can occasionally produce hydrologically undesirable results, for example reversals in the response relation on either side of the zero-slope value, or extraordinarily large response values of extrapolated  outside of the calibration data. Helsel and Hirsch, 2002, discuss the ladder-of-powers transformation, which can sometimes be used to reduce the likelihood of the reversals. The \texttt{boxCox} function in \texttt{USGSwsBase} can be used to construct several candidate models, selecting the "best" fit for the power.
...
<<fig=TRUE, echo=FALSE>>=
with(DF, plot(Flow, Y2, type='p', pch=16, tck=.02, log="xy"))
...
<<echo=TRUE>>=
# The model with the hyperbolic transform
lm(log10(Y1) ~ sCurve(log10(Flow), location=log10(1200)), data=DF)
@

<<fig=TRUE, echo=FALSE>>=
with(DF, plot(Flow, Y2, type='p', pch=16, tck=.02, log="xy"))
curve(10^(-0.03854 + 0.57632 * sCurve(log10(x), location=log10(1200))),
      166, 7700, add=TRUE)
@
...
Helsel and Hirsch, 2002, describe multiple regression with periodic functions in section 12.4.3. They describe the approach by adding two variables, one for the sine and one for the cosine. The \texttt{fourier} function in \texttt{USGSwsBase} simplifies the addition of those variables by adding both transforms to the model. It also forces both or neither to be considered in best-subset selection procedures. The \texttt{k.max} argument can be used to add higher-order seasonal terms.
...
<<fig=TRUE, echo=FALSE>>=
with(DF, plot(Date, Y3, type='p', pch=16, tck=.02, log="y"))
