...
The example above can easily be expanded to any grouping that the user desires. This example computes annual means by calendar year. The \texttt{year} function in \texttt{lubridate} is used to group the data by calendar year. The \texttt{waterYear} function in \texttt{smwrBase} can be used to group the data by water year.
...
Other grouping functions include \texttt{month} (month) in \texttt{lubridate}, \texttt{seasons} (user-defined seasons) in \texttt{smwrBase}. Refer to the documentation for each of these function for a description of the arguments.
